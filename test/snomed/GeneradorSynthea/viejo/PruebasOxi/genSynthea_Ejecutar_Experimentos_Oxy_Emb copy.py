...
with open(f"{path}/tiempos.csv", "w", encoding="utf-8") as f:
    ...
    for nEvents in N_TRAZAS:
        for nt in LONGS:
            ...
            print("He generado my_propositions.py")
            ...
            # 2. EJECUTAR MC (CORREGIDO)
            cmd = [
                "python",
                "MC.py",
                f"log-file={path_auto}/log_{nEvents}_{nt[0]}_{nt[1]}",
                "interactive=false",
                f"formula-file={path}/formulas.txt"
            ]

            res_file = f"{path_auto}/log_{nEvents}_{nt[0]}_{nt[1]}_res.txt"
