...
def get_deep_semantics(state_name, st_data, event_uri, wf_name):
    ...
    # --- RECOLECTOR MASIVO DE CÓDIGOS (Busca en TODO el estado) ---
    all_raw_codes = []

    # 1. Bloque estándar de códigos
    all_raw_codes.extend(st_data.get("codes", []))
    
    # 2. Códigos en Actividades (Típico en CarePlans y Colonoscopias)
    for act in st_data.get("activities", []):
        all_raw_codes.extend(act.get("codes", []))
    
    # 3. Códigos en Observaciones (Resultados de laboratorio, biopsias)
    for obs in st_data.get("observations", []):
        all_raw_codes.extend(obs.get("codes", []))
    
    # 4. Códigos de Medicación (RxNorm/SNOMED en Prescription)
    if "medication_code" in st_data:
        all_raw_codes.append(st_data["medication_code"])
    
    # 5. Códigos de salida y valores directos
    if "value_code" in st_data:
        all_raw_codes.append(st_data["value_code"])
    if "discharge_disposition" in st_data:
        all_raw_codes.append(st_data["discharge_disposition"])
    
    # 6. Mapeo semántico de 'Reason' (Si existe un código de razón)
    if "reason" in st_data:
        # A veces el 'reason' es un código directo en Synthea
        q.append(f"{proc_uri} <http://example.org/ontology/reasonCode> \"{clean(st_data['reason'])}\" {event_uri} .")

    # --- PROCESAMIENTO Y GENERACIÓN DE TRIPLES ---
    # Usamos un set para evitar duplicar el mismo código en el mismo evento
    processed_uris = set()
    ...
    for c in all_raw_codes:
        ...
        # Generación de la URI según el sistema
        if "SNOMED" in sys.upper():
            c_uri = f"<http://snomed.info/id/{code}>"
        elif "LOINC" in sys.upper():
            c_uri = f"<http://loinc.org/rdf/{code}>"
        elif "RXNORM" in sys.upper():
            c_uri = f"<http://rxnav.nlm.nih.gov/REST/rxcui/{code}>"
        else:
            c_uri = f"<http://example.org/code/{sys}/{code}>"

        if c_uri not in processed_uris:
            # Triple: El procedimiento tiene este código clínico
            q.append(f"{proc_uri} <http://example.org/ontology/clinicalCode> {c_uri} {event_uri} .")
            # Triple: Etiqueta del código para que sea legible
